...
# Initial gaussian on sphere (t=0 condition)
...
# wave dataset for spherical domain
...
def loss_pinn_sphere(model_hat, X_f, X_ic,c, beta_f=1.2, beta_ic=1.2, sigma=0.2,theta0=1.0, phi0=1.0, R=1.0):
    # PDE loss (on all the interior domain points = X_f)
    # ----------------------
    X_f.requires_grad_(True)  #activated so we can compute the gradients 
    theta_f = X_f[:,0:1]
    phi_f   = X_f[:,1:2]
    u_f = model_hat(X_f)

    grads_f = torch.autograd.grad(u_f, X_f,grad_outputs=torch.ones_like(u_f),retain_graph=True, create_graph=True)[0]
    u_t = grads_f[:,2:3]
    u_tt = torch.autograd.grad(u_t, X_f,grad_outputs=torch.ones_like(u_t),retain_graph=True, create_graph=True)[0][:,2:3]
    u_theta = grads_f[:,0:1]
    u_phi   = grads_f[:,1:2]
    u_theta_theta = torch.autograd.grad(u_theta, X_f,grad_outputs=torch.ones_like(u_theta),retain_graph=True, create_graph=True)[0][:,0:1]
    u_phi_phi = torch.autograd.grad(u_phi, X_f,grad_outputs=torch.ones_like(u_phi),retain_graph=True, create_graph=True)[0][:,1:2]
    ...
    # init. wave state condition
    loss_u0 = torch.mean((u_ic - u_ic_exact)**2)
    # init. velocity (like in 1D and 2D, we assume the wave starts from rest i.e. v_0=0)
    u_t_ic = torch.autograd.grad(u_ic, X_ic,grad_outputs=torch.ones_like(u_ic),retain_graph=True, create_graph=True)[0][:,2:3]
